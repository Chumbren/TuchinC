    ...
    internal class Scanner
    {
        ...
        private void ScanToken()
        {
            ...
            switch (c)
            {
                ...
                case '-': AddToken(Match('>') ? TokenType.ARROW_OPERATOR : TokenType.MINUS); break;
        ...
        private void Identifier()
        {
            ...
            if (!_keywords.TryGetValue(text, out TokenType type))
                type = ValueType.PrimitiveTypes.Contains(text) ? TokenType.LITERAL : TokenType.IDENTIFIER;
        ...
        {
            if (float.TryParse(source, out var d32))
                return d32;

            if (double.TryParse(source, out var d64))
                return d64;

            if (decimal.TryParse(source, out var d128))
                return d128;
