...
{
    internal partial class TypeAnalizator : IVisitor<object?>
    {

        public void Analize(Expr? expr) => expr?.Accept(this);
        ...
        public object? VisitAssignExpr(Assign expr)
        {
            ...
            if (!TryGetPrimitiveType(name, out TypeValue type))
                throw new ArgumentOutOfRangeException(nameof(expr), expr, "Не правельно прошедшая проверка resolver");
            


            var initIndex = EmitWait();

            Analize(expr.Value);

            var init = Index(initIndex);

            if (!CastDeclaration.CanCast(type, init.Type))
                throw new CastBinnaryException(init, new ValueType(type));
        ...
        private void Binnary(Binary binary)
        {
            int lIndex = EmitWait();
            int rIndex = EmitWait();
            Analize(binary.Right);
            Analize(binary.Left);
            ValueType lType = Index(lIndex);
            ValueType rType = Index(rIndex);

            EmitQuit(CastBinnaryType(ref lType, ref rType, binary.Operator.Type));


            if (ArithmeticCaster.Cast(lType, rType, binary.Operator.Type).Type == TypeValue.None)
                Tuchin.Error(binary.Operator,
                     new NotCheckBinnaryOperationException(binary.Operator.Lexeme, lType, rType
                     ));
        }
        ...
        public object? VisitLogicalExpr(Logical expr)
        {
            ...
            if (!LogicalCaster.CanCast(lType, rType, expr.Operator.Type))
                Tuchin.Error(expr.Operator,
                     new NotCheckBinnaryOperationException(expr.Operator.Lexeme, lType, rType
                     ));
            
            EmitQuit(ValueType.ToBoolean());
        ...
        {
            if (!TryGetPrimitiveType(expr.Name.Lexeme, out TypeValue type))
                throw new ArgumentOutOfRangeException(nameof(expr), expr, "Переменая не существует, Resolver сработал ошибочно");

            EmitQuit(new ValueType(type));
